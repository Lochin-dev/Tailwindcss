    ...
    <div className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexCenter} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          <img src={logo} alt="logo" className="w-[300px] h-[122px] " />
          <p className={`${styles.pargraph} mt-4 max-w-[350px] `}>
            To'lovlarni oson, ishonchlo va xavsiz qilishning yangi usuli
          </p>
        </div>
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((social, indx) => (
            <div
              key={indx}
              className={`${
                indx === socialMedia.length - 1 ? "mr-0" : "mr-6"
              } text-white`}
            >
              {social.icon}
            </div>
